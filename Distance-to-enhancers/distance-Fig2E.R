...
down <- read.table("Down_promoter_nearest_OSN.txt",stringsAsFactors=F)
down <- down[down$V1 == down$V7,]

up <- read.table("Up_promoter_nearest_OSN.txt",stringsAsFactors=F)
up <- up[up$V1 == up$V7,]

nochange <- read.table("Nochange_promoter_nearest_OSN.txt",stringsAsFactors=F)
...
ks.test(jitter(all4$fre[all4$type=="Up"]),jitter(all4$fre[all4$type=="Down"]))     #2.121e-05
ks.test(jitter(all4$fre[all4$type=="Nochange"]),jitter(all4$fre[all4$type=="Down"]))  #9.588e-05
ks.test(jitter(all4$fre[all4$type=="Nochange"]),jitter(all4$fre[all4$type=="Up"]))  #0.03453

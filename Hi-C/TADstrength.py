...
with open(sys.argv[3]) as infile:
	for line in infile:
		...
		bins_start = bininfo[chrom_start_info]
		bins_end = bininfo[chrom_end_info]

		bin_length = bins_end - bins_start 

		matrix2 = np.full((bin_length , bin_length ), np.nan)

		for i in range(bins_start,bins_end):
			for j in range(bins_start,bins_end):
				distance = abs(i - j)
				index_i = i - bins_start
				index_j = j - bins_start
				strs = str(i) + '_' + str(j)

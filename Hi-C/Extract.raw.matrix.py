...
chrom_start_info = locus.split(':')[0] + '-' + locus.split(':')[1].split("-")[0] + '-' + str(int(locus.split(':')[1].split("-")[0])+binsize)
chrom_end_info = locus.split(':')[0] + '-' +  str(int(locus.split(':')[1].split("-")[1])-binsize) + '-' + locus.split(':')[1].split("-")[1]
	...
	for j in range(bins_before,bins_after):
		...
		if index_j > index_i:
			...
			if strs in interaction:
				matrix[index_i,index_j] = float(interaction[strs]) #/ float(distance_x2[locus.split(':')[0]][distance])
for i in range(bins_before,bins_after):
	for j in range(bins_before,bins_after):
		distance = abs(i - j)
		index_i = i - bins_before
		...
		if index_j > index_i:
			...
			if strs in interaction2:
				matrix2[index_i,index_j] = float(interaction2[strs])  #/ float(distance2[locus.split(':')[0]][distance])

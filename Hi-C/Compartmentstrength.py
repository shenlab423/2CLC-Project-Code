...
def meanArray(array):
	matrixout = np.full((5 , 5 ), np.nan)
	if len(array[0]) >= 5:
		splits = split_integer(len(array[0]),5)
		i = 0
		for m in range(5):
			j = 0
			for n in range(5):
...
for chrom,order in compartment_chr.items():
	...
	index += 1
	print("%s\t%s\t%s\t%s\t%s"%(chrom,matrix3[0,0],matrix3[0,4],matrix3[4,0],matrix3[4,4]))
